...
<head>
  ...
  <style>
    ...
    .find_id_section form p {
      display: block;
      margin: 20px auto;
    }

    .find_id_section form label {
      font-size: 0.875rem;
      line-height: 30px;
      width: 20%;
      float: left;
    ...
    .find_id_section form input:focus {
      outline: none;
    }

    .find_id_section form span {
      display: block;
      width: 100%;
      float: left;
      font-size: 0.75rem
    ...
    .find_id_section form button:hover {
      cursor: pointer;
      background: #ff5454;
    ...
    .find_id_section form button:active {
      color: #f8b5b0;
...
<body>
  ...
  <section class="find_id_section">
    ...
    <form id="id_form" action="find_look.php" method="POST" onsubmit="return form_check()">
      <fieldset>
        <legend class="blind">아이디 찾기</legend>

        <p class="cfixed">
          <label for="mem_nm"">이름</label>
          <input type=" text" name="mem_nm" id="mem_nm" size="10" placeholder="이름을 입력해 주세요" autocomplete="off"
            autocapitalize="off">
        ...
        <p class="cfixed">
          ...
          <input type="text" name="tel_no" id="tel_no" size="11" placeholder="숫자만 입력해 주세요 ex)01033330000"
